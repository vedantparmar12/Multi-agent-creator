...
- **📚 Context Engineering**: Agents leverage project-specific context, rules, and examples for better performance
- **🎯 PRP System**: Generate and execute Product Requirements Prompts for complex implementations
- **✅ Validation Loops**: Self-correcting agents with automated testing and linting
...
Make It Heavy now includes powerful context engineering capabilities that dramatically improve agent performance:

### What is Context Engineering?

Context Engineering provides agents with comprehensive project context including:
...
- **Examples**: Code patterns for agents to follow
- **PRPs**: Product Requirements Prompts for complex features

### Using Context-Aware Agents

```python
# Agents automatically load context
from agent import OpenRouterAgent

agent = OpenRouterAgent(context_aware=True)
response = agent.run("Show me the project rules")
# Agent will reference CLAUDE.md and other context files
```
...
User: "Generate a PRP for adding Redis caching to the system"
Agent: [Uses manage_prp tool to create detailed implementation plan]
```

### Context Tools

New tools available to agents:
- **load_project_context**: Access project rules and documentation
- **manage_prp**: Generate or execute PRPs
- **run_validation**: Lint, typecheck, and test code

### Project Structure with Context

```
make-it-heavy/
├── CLAUDE.md          # Project rules (AI follows these)
...
## 📝 Best Practices with Context Engineering

### 1. Create Good Examples

Place example code in the `examples/` directory:
```python
# examples/api_client_pattern.py
"""Pattern for creating API clients."""
...
class APIClient:
    ...
    async def request(self, endpoint: str) -> dict:
        # Pattern agents should follow
        pass
```

### 2. Write Clear CLAUDE.md Rules
...
### 3. Use PRPs for Complex Features

For multi-file changes, generate a PRP:
```bash
User: "Generate a PRP for adding user authentication with OAuth"
Agent: [Creates comprehensive blueprint with all steps]
```

### 4. Leverage Validation

Agents can self-correct with validation:
```python
User: "Implement the new feature and ensure all tests pass"
Agent: [Implements, runs tests, fixes issues, validates]
```
